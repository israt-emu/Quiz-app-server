...
import {IBooksWithMeta} from "./book.interface";

export const createBookService = async (payload: Book): Promise<Book> => {
...
export const getAllBookService = async (req: Request): Promise<IBooksWithMeta> => {
...
export const getSingleBookService = async (id: string): Promise<Book | null> => {
...
export const updateBookService = async (id: string, payload: Partial<Book>): Promise<Book | null> => {
  const book = await prisma.book.findUnique({
    where: {
      id,
    },
